"""
Sector Command — Twilio Webhook (Vercel Serverless Function)
...
Commands:
    BUY          → Execute latest buy recommendation at full size
    BUY $30      → Execute buy at custom dollar amount
    SELL         → Execute latest sell recommendation
    SELL ALL XLK → Sell entire position in XLK
    SKIP         → Log signal as declined
    STATUS       → Reply with current portfolio breakdown
    WHY          → Reply with detailed reasoning
    PAUSE        → Pause all signals
    RESUME       → Resume signals
    FORCE SELL ALL → Liquidate everything to cash
"""
...
# Note: In Vercel, these are installed via requirements.txt in the project root
# For local testing, pip install twilio gspread google-auth
...
def handler(request):
    ...
    if request.method != "POST":
        return {"statusCode": 200, "body": "Sector Command Webhook Active"}

    # Parse Twilio webhook payload
    body = request.form if hasattr(request, 'form') else {}
    from_number = body.get("From", "")
    message_body = body.get("Body", "").strip().upper()
    timestamp = datetime.now().isoformat()

    print(f"[Webhook] {timestamp} | From: {from_number} | Message: {message_body}")

    # Authenticate - only accept from Cameron's number
