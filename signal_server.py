...
class SectorCommand:
    ...
    def format_sms_full(self, signals, portfolio=None):
        ...
        if not recs:
            ...
            for r in recs[:3]:
                emoji = "BUY" if r["action"] == "BUY" else "SELL"
                vetoed_tag = " [VETOED]" if r["vetoed"] else ""
                if portfolio:
                    bal = portfolio.get("balance", 400)
                    dollar_amt = bal * r["target_weight"] * r["sizing_multiplier"]
                    lines.append(f"{emoji} ${dollar_amt:.0f} {r['ticker']} ({r['sector'][:12]}){vetoed_tag}")
                else:
                    lines.append(f"{emoji} {r['ticker']} -> {r['target_weight']:.1%}{vetoed_tag}")
                lines.append(f"  Conviction: {r['conviction_label']} ({r['conviction']}/3)")
                lines.append(f"  Risk: {r['risk']} | RSI: {r['rsi']:.0f} | b: {r['beta']:.1f}")
                if r["vetoed"]:
